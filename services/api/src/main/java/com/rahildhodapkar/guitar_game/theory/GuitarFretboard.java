...
public final class GuitarFretboard {
    ...
    public List<PlayedNote> notesFromFrets(List<Integer> frets) {
        if (tuning.numberOfStrings() != frets.size()) {
            throw new IllegalArgumentException("Input frets size does not match the number of guitar strings");
        }

        for (int fret : frets) {
            if (fret < MUTED_STRING || fret > maxFrets) {
                throw new IllegalArgumentException("Input fret value of " + String.valueOf(fret) + " is invalid");
            }
        }
    ...
    private PlayedNote getActualNotePlayed(PlayedNote openString, int fretPlayed) {
        ...
        return new PlayedNote(transposedPitchClass, transposedPitchClass.name(false), frettedMidiNumber);
    }
}
